  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-hidden p-6 select-none">
      <div className="mb-6 pb-4 border-b border-border-subtle">
        <h1 className="text-2xl font-black text-foreground tracking-tight">Artists</h1>
      ...
      <div className="flex-1 overflow-y-auto pr-1">
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 pb-6">
          {filteredArtists.map((artist) => (
            ...
            >
              {/* Avatar circle */}
              <div className="relative w-28 h-28 rounded-full overflow-hidden bg-surface-input mb-3 shadow-lg border border-border-subtle flex items-center justify-center">
                {artist.artworkUrl ? (
                  <img
                  ...
                  <Users className="w-10 h-10 text-foreground-subtle transition" />
                )}
              </div>

              <h3 className="text-xs font-bold text-foreground truncate w-full group-hover:text-accent-text transition">
                {artist.name}
              </h3>
